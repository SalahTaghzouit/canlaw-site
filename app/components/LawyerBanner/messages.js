/*
 * LawyerBanner Messages
 *
 * This contains all the text for the LawyerBanner component.
 */
import { defineMessages } from 'react-intl';

export default defineMessages({
  header: {
    id: 'canlaw.site.components.LawyerBanner.header',
    defaultMessage: 'This is the LawyerBanner component !',
  },
  grow: {
    id: 'canlaw.site.components.LawyerBanner.grow',
    defaultMessage: 'Grow Your Law Practice. Expand Your Network.',
  },
  growAlt: {
    id: 'canlaw.site.components.LawyerBanner.grow',
    defaultMessage: 'Grow your business with Canlaw',
  },
  nocharge: {
    id: 'canlaw.site.components.LawyerBanner.nocharge',
    defaultMessage: 'No charge. No commission. No fee-splitting. No touting.',
  },
  readmore: {
    id: 'canlaw.site.components.LawyerBanner.readmore',
    defaultMessage: 'Find Out More',
  },
  signUpLawyer: {
    id: 'canlaw.site.components.LawyerBanner.signUpLawyer',
    defaultMessage: 'Sign up as a lawyer',
  },
});
